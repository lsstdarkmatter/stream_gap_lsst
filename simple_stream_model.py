...
def gamma(R,z):
    if not isinstance(R,u.Quantity): R= R*u.kpc
    if not isinstance(z,u.Quantity): z= z*u.kpc
...
def gap_size(Mhalo,dist=10.*u.kpc,
             ...
             w=150.,timpact=1.,R=0.*u.kpc,z=20.*u.kpc):
    """
    NAME:
       gap_size
    PURPOSE:
    ...
    tcaustic= 4.**tgamma**2./(4.-tgamma**2.)*wwperpfac**2.*w*(X**2.+1.)\
        *rs(Mhalo)**2./G/Mhalo
    B= numpy.sqrt(X**2.+1.)*rs(Mhalo)*wwperpfac/dist
    if timpact < tcaustic:
...
def gap_depth(Mhalo,wwperpfac=numpy.sqrt(3./2.),
              w=150.,X=1.,timpact=1.,R=0.*u.kpc,z=20.*u.kpc,**kwargs):
    """
    NAME:
       gap_depth
    PURPOSE:
    ...
    if not isinstance(z,u.Quantity):
        z= z*u.km/u.s
    tgamma= gamma(R,z)
    return 1./(1.+(4.-tgamma**2.)/tgamma**2./wwperpfac**2./w*2.*G*Mhalo/(X**2.+1.)/rs(Mhalo)**2.*timpact).value
